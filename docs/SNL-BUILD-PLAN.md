...
| Comparable Deal Analysis | **SHIPPED 2026-07-13** (owner: 'the best part'; warm-job + dual-TBV-basis + all four analytics owner-decided): data/deal_comps.py — P/TBV paid / price-assets / core-deposit-premium per priced deal; TBV basis = SEC holdco TCE (audited sec_per_share machinery, new tangible_common_equity_at) for the RATIO'S priced entity (ma_history v6 target_cik — the Columbia/Umpqua bank-vs-holdco FLIP guard: $5.19B pairs with UMPQ TCE = 1.91x, never Columbia State Bank 3.4x plausible-wrong), FDIC bank-sub EQ-INTAN otherwise; CDP bank-sub-basis-only (COREDEP>10% assets); 0.2-8x sanity band flags basis mismatches. jobs/refresh_deal_comps walks the universe nightly (+ deploy.yml 3 job loops), compiles ONE snapshot; UI reads snapshot only — filters, median-P/TBV-by-year chart, size buckets, log scatter + selected-bank overlay, 250-cap table w/ basis tooltips + export. GROUND TRUTHS HIT: Skagit 2.40x vs Banner's own deck 2.37x (basis delta), Columbia/Umpqua 1.91x vs press ~1.9-2x, COLB/PPB 0.98x vs reported ~0.95-1x; CDPs 3.9-20.6% classic ranges. PENDING GAP CLOSED 2026-07-14 (data/ma_pending.py, ma_history v7): Rule 425 episode detection (form-driven; latest cluster <540d = live deal), Subject Company legend = authoritative target (self-detection by universe-CIK identity, NOT name tokens — 'Tri Counties Bank' vs 'TriCo Bancshares'), counterparty universe-matched to cert/CIK, value stated-or-computed w/ comma-tolerant ratio forms (upstream into ma_announcements once the 404-fix session frees the file). Live-verified DAY-ONE: FHB/TriCo announced 2026-07-13 -> pending row $2.014B computed (2.095 x FHB $30.13 = the PR's own $63.12/share), TCBK assets $9.95B, comps P/TBV 2.11x holdco. Pending sale-side rows skipped in comps (acquirer's row carries); dedup vs completed/terminated by counterparty token. CASH-DEAL LEG BUILT 2026-07-14 (ma_history v8): cash pending deals file NO 425 — ma_announcements.find_open_announcements sweeps the filer's recent announce-classified 8-Ks (<540d, no completion/termination anchor), counterparty = non-self ticker pair else the acquire-verb object for PRIVATE targets (longest capture per brand token — headline full name beats body short name; recurrence >=2 required, never a guess); self-exclusion is word-boundary containment over the filer's OWN EFTS display names (CIK identity), NOT brand-token equality — pins the live bug where empty subject_name (bank with no FDIC structure rows) let 'About Catalyst Bancorp...' token to 'about' and leak SELF as counterparty; get_ma_history(name=) fallback plumbed from all three callers. ma_pending merges both legs (dedup by counterparty token, 425 wins). Live-verified: CLST/Lakeside Bancshares (private, zero 425s) $41.1M stated 'in aggregate' both with and without subject name; FHB dedupes to exactly 1 TriCo row across legs; Banner control 0. Prod job created + nightly 7:30am ET scheduler LIVE; first snapshot 2550 deals/269 priced/323 banks |
