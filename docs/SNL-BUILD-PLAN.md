...
| Sub-tab | Source |
|---|---|
| Transactions Summary (CONFIRMED via screenshots) | **SHIPPED 2026-07-13**: Aggregate/Details toggle (Details = shared deal-table renderer); volume chart (M&A value bars + all-transaction count line, multi-decade, standard chart system); Top Transactions by Value; type pie (whole-co M&A / branch / terminated from deal data + Offerings 424B [ECM/DCM unsplit until Detailed Offerings] / Shelf S-1,S-3 family / Buyback 8-Ks — owner-approved filing-type buckets, own classification labeled); buyback feed = holdco 8-Ks quoting 'repurchase program' ex-earnings-item-2.02 (data/ma_summary.py, EFTS 2001+ paged, full submissions history paged, cache ma_summary:v1 7d, fetch-failure uncacheable). Shared bank picker (one key) across Summary + M&A History. Render-verified live: Banner pills 19/18/13/12, top deal Skagit $191.1M, feed head = real 2025-07-24 authorization, Details = 19-row table. Authorized buyback amounts + ECM/DCM split land with Detailed Offerings. ~~Top Advisers panels~~ SKIPPED (user X) |
| Detailed M&A History (CONFIRMED) | deal table: announce + completion dates, target, buyer, seller, value $M, acquisition vs sale, type (whole company / asset-or-branch), TARGET TOTAL ASSETS at announcement (computable from FDIC — incl. branch deals + terminated/withdrawn). Sources: FDIC structure events (completed) + events-store M&A detection + 8-K/S-4/PR (announcements, values, terminations). **FDIC completed-deals leg BUILT 2026-07-13** (`data/ma_history.py` `get_ma_history(cert)`): whole-company deals both directions (810/811/812 + terminal 2xx) with target total assets at last REPDTE ≤ completion ($k→raw dollars at boundary; non-SDI targets n/a), branch-package purchases AND sales with exact branch counts (712 header + 722 office rows — both recorded on the BUYER's cert; sales recovered via `OUT_CERT:{cert}` reverse query; taxonomy verified live: 713 = whole-bank echo, not a deal). Value-verified: Umpqua/Columbia 2023 ($20,258,988k at 2022-12-31), Banner↔Umpqua 2014 six-branch divestiture from both certs, Banner's four SNL-screenshot deals. **Announcements leg increment A BUILT 2026-07-13** (`data/ma_announcements.py`, wired into `get_ma_history`, cache key bumped v2): announce date + STATED value from the announcement 8-K via EDGAR full-text search (2001+ floor, clamped), whole-company deals with SDI-real-bank guard only (prevents the verified Columbia-Trust affiliate mislinkage); strict both-party-name + announce-vs-completion-marker gates, items prefilter (modern codes only), value verbatim-verified on 6 deals (Skagit $191.1M, PPB $2.0B, AltaPacific $87.4M, Siuslaw $57.5M, NCW $18.5M, OBB $10.0M); misses are honest n/a (e.g. Sterling — PR used rebranded trade name). **Increment B BUILT 2026-07-13** (cache v3): computed all-stock value for ratio-only MOE PRs — ratio x acquirer prior close (FMP EOD) x target cover shares (companyfacts dei nearest<=announce; delisted-target CIK via EFTS display-name fallback), value_basis='computed' + value_note formula verbatim; live-verified Columbia/Umpqua $5,189,818,466 vs press ~$5.2B; strict n/a on ambiguous ratio/unmapped party/stale shares(>200d)/stale price(>10d). **Increment C BUILT 2026-07-13** (cache v4, status field on every row): terminated/withdrawn deals via per-holdco-CIK EFTS sweep — 8-K groups with item 1.02 citing 'Agreement and Plan of Merger', back-linked EARLIEST-first to the original announcement (extensions can't steal the date), counterparty from union of both PRs' ticker pairs cross-checked in both docs, EX-99 exhibits index-fetched (the PR rarely contains the legal phrase); get_ma_history(cert, cik=) merges status='terminated' rows; live-verified FHN/TD (announced 2022-02-28 US$13.4B stated, terminated 2023-05-04; Banner control = zero). Unlinkable terminations DROPPED, never guessed. **UI SUB-TAB SHIPPED 2026-07-13** (ui/transactions.py rebuilt): top-level Transactions = lazy_tabs [Detailed M&A History, Insider Activity(kept, unchanged)], further §14 tabs added as built (no placeholders). M&A pane: bank picker (owner-decided scope; universe view = later warm-job increment), KPI pills, dense ksk-grid (announced/completed/status/target/buyer/seller/type/value/target-assets), EDGAR-linked announce dates, cert->ticker company deep-links (fdic_cert; fail-open), value hover = computed formula, assets hover = as-of REPDTE, terminated rows red-badged. Target assets re-anchored AT ANNOUNCE date when resolved (SNL convention; cache v5; Columbia $18,597,100k @2021-09-30 hand-verified). Render-verified live in preview: Banner 19 rows (Skagit $191.1M, AltaPacific $87.4M, Branch(6) w/ Umpqua->COLB deep-link, pills 19/16/6/0) + FHN 47 rows incl. the terminated TD $13.40B row; AppTest guard tests/test_transactions_ui.py. Transactions Summary sub-tab SHIPPED (volume chart + top-deals + type pie + buyback feed, ui/transactions._render_summary + data/ma_summary; marker was stale — corrected 2026-08-31 after nearly rebuilding it) |
| Detailed Offerings | **SHIPPED 2026-07-13** (layout owner-confirmed + PP leg added): data/offerings.py per-holdco — 424B covers fetched + per-document classified (Merger prospectus [S-4 deal docs — a 424B is NOT always an offering, live-verified] / Preliminary / Resale-selling-holder [TARP Treasury auctions, no company proceeds] / ECM / DCM / Unclassified [pre-2001 text filings]); gross strict-extracted (DCM 'aggregate principal amount'; ECM SEC cover price-table [per-share, total] with total>>price sanity + shares x lone-price fallback); S-1/S-3 shelf rows; 8-K Item 3.02 private placements (3.02+2.01 skipped = acquisition consideration; gross ambiguity n/a). UI: shared picker, pills (known gross raised/ECM/DCM/PP/shelf), dense table w/ EDGAR links. Summary pie now splits Offerings via this classification (fixes the 424B overcount). Hand-verified Banner: $150M @ $2.00 June-2010 recap ECM, $100M 2020 sub notes DCM, TARP resale, 4 merger prospectuses, $250M known raised. Cache offerings:v1 7d, failure uncacheable |
| Private Equity Transactions | **SHIPPED 2026-07-13** (layout owner-confirmed: 13D prominent, 13G collapsed): data/stake_filings.py — authoritative SC 13D/13G list from issuer submissions (complete incl. pre-2001) + holder names joined from EFTS display_names by accession (BRAND-TOKEN query — a quoted 'Banner Corp' phrase misses 'Banner Corporation' covers, live-verified; pre-2001 rows keep honest '—' holder + filing link). 13D vs 13G is the form's own distinction, no classification of ours. UI: 13D table (activist/control) + private placements reused from the Offerings leg (TARP CPP surfaces naturally) + 13G pile behind an expander. Live-verified Banner: 8 13Ds led by Oaktree + Friedman Fleischer & Lowe (the AmericanWest-deal PE holders) + Klaue 2007; 102 13Gs (Vanguard/BlackRock/State Street/DFA/Wellington); 101/110 holders named. Cache stake_filings:v1 7d, failure uncacheable |
| Comparable Deal Analysis | **SHIPPED 2026-07-13** (owner: 'the best part'; warm-job + dual-TBV-basis + all four analytics owner-decided): data/deal_comps.py — P/TBV paid / price-assets / core-deposit-premium per priced deal; TBV basis = SEC holdco TCE (audited sec_per_share machinery, new tangible_common_equity_at) for the RATIO'S priced entity (ma_history v6 target_cik — the Columbia/Umpqua bank-vs-holdco FLIP guard: $5.19B pairs with UMPQ TCE = 1.91x, never Columbia State Bank 3.4x plausible-wrong), FDIC bank-sub EQ-INTAN otherwise; CDP bank-sub-basis-only (COREDEP>10% assets); 0.2-8x sanity band flags basis mismatches. jobs/refresh_deal_comps walks the universe nightly (+ deploy.yml 3 job loops), compiles ONE snapshot; UI reads snapshot only — filters, median-P/TBV-by-year chart, size buckets, log scatter + selected-bank overlay, 250-cap table w/ basis tooltips + export. GROUND TRUTHS HIT: Skagit 2.40x vs Banner's own deck 2.37x (basis delta), Columbia/Umpqua 1.91x vs press ~1.9-2x, COLB/PPB 0.98x vs reported ~0.95-1x; CDPs 3.9-20.6% classic ranges. PENDING DEALS REBUILT 2026-07-16 with OPEN-STATUS VERIFICATION (ma_history v9; the first cut was REVERTED 2026-07-15 for showing CLOSED deals as pending — presence of 425s/announcement 8-Ks persists after close and proves nothing): data/ma_pending two legs (Rule 425 episodes for stock deals; ma_announcements.find_open_announcements 8-K sweep for cash deals, completion-8-K groups w/ item 2.01 excluded at source) merged + deduped, then EVERY candidate passes TWO nets before display — SEC net: a later 8-K item 2.01/1.02 naming the counterparty (brand token, or full phrase for all-generic names like 'American Bank Holding Company') or an 8.01 naming it in COMPLETED tense (item discipline varies: Hope filed the Territorial completion under 8.01 only) resolves the deal, with a 45d pre-anchor slack (post-close 8-K latched as announcement — UMBF class); unverifiable (no needle / fetch fail) = DROPPED never shown; FDIC net in ma_history: pending row retired by a completed/terminated row matching brand token or leading-two-words (tokenless names — FDIC recorded Prosperity/American as 'American Bank, National Association', NO 2.01 ever filed). Counterparty names cleaned of 'About X. X' footer run-ons (sentence-boundary split, U.S. Bancorp-safe). E2E ground-truth battery (all primary-source-confirmed): CLST/Lakeside + PB/Stellar + PB/American + FULT/Blue Foundry + UMBF/Heartland + HOPE/Territorial all closed -> ZERO pending rows; FHB/TriCo open -> exactly 1 row each side ($2,014,271,431 computed, hand math); Banner control 0. build_comps_snapshot per-bank guard added (un-guarded EDGAR throw crashed the job 2026-07-15). Job needs one-time Cloud Run create + scheduler (DONE — nightly 7:30am ET live) |
