...
def as_reported_statement_multiquarter(cik, stype: str = "income",
    ...
    if not q_metas:
        return None
    ckey = f"asreported_mq:v5:{stype}:{q_metas[0]['accession']}:{n_quarters}"  # v5: member blocks + Q4 vintage
...
def as_reported_statement_multiyear(cik, stype: str = "income", n_years: int = 5) -> dict | None:
    ...
    if metas[0].get("date", "") < (date.today() - timedelta(days=540)).isoformat():
        return None
    ckey = f"asreported_my:v9:{stype}:{metas[0]['accession']}:{n_years}"  # v9: member blocks
    cached = cache.get(ckey, max_age_s=None)
    if cached is not None:
        return cached or None
    parsed = []
