        ...
        if _USE_POSTGRES:
            sql = text("""
                INSERT INTO price_cache
                  (ticker, price, prev_close, change, change_pct, volume, updated_at)
                VALUES (:ticker, :price, :prev_close, :change, :change_pct, :volume, NOW())
            ...
            sql = text("""
                INSERT OR REPLACE INTO price_cache
                  (ticker, price, prev_close, change, change_pct, volume, updated_at)
                VALUES (:ticker, :price, :prev_close, :change, :change_pct, :volume,
                        CURRENT_TIMESTAMP)
            """)
            for r in rows:
                conn.execute(sql, r)
    ...
    with eng.begin() as conn:
        ...
        for i in range(0, len(up), 500):
            ...
            rows = conn.execute(text(
                ...
                f"updated_at FROM price_cache WHERE ticker IN ({placeholders})"
            ), params).fetchall()
            for r in rows:
