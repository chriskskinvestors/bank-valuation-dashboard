"""
One-shot: create the refresh-prices + verify-metrics Cloud Run jobs and their
Cloud Schedulers, cloning the existing refresh-universe job config (image, SA,
Cloud SQL volume, DATABASE_URL) and the poll-events scheduler pattern.

Run AFTER a deploy that includes jobs/refresh_prices.py + jobs/verify_metrics.py
(so the cloned image digest contains them). Uses ADC. Idempotent: skips
anything that already exists.

  PYTHONIOENCODING=utf-8 python -X utf8 tools/create_price_jobs.py
"""
...
# New jobs: (job_id, module, optional scheduler (cron, tz))
NEW_JOBS = [
    ("refresh-prices", "jobs.refresh_prices",
     ("*/2 9-16 * * 1-5", "America/New_York")),     # warm prices, market hours
    ("verify-metrics", "jobs.verify_metrics",
     ("0 18 * * 1-5", "America/New_York")),          # nightly accuracy check
]
...
def _job_exists(s, job_id) -> bool:
    r = s.get(f"{RUN}/jobs/{job_id}")
    return r.status_code == 200


def _create_job(s, job_id, module, template):
    """Clone an ExecutionTemplate, point it at `module`, add FMP secret."""
    tmpl = copy.deepcopy(template)
    container = tmpl["template"]["containers"][0]
    container["command"] = ["python"]
    container["args"] = ["-m", module]
    # Ensure FMP_API_KEY is mounted (price fetch + price-dependent verify).
    ...
    body = {"template": tmpl}
    r = s.post(f"{RUN}/jobs?jobId={job_id}", json=body)
...
def _fire(s, job_id):
    ...
    if r.status_code in (200, 201):
        ex = r.json().get("metadata", {}).get("name", "")
        print(f"  ✓ fired {job_id}: {ex.split('/')[-1]}")
        return True
...
def main() -> int:
    s = _session()
    # Clone the template from refresh-universe (now on the freshly-deployed image).
    ...
    # Warm the price cache immediately so screens are fast right away.
    print("\nFiring initial warm-up:")
    _fire(s, "refresh-prices")
    return 0
