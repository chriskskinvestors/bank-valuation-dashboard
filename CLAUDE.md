...
**Shared infrastructure (use these, never re-add local copies):** `data/http.py` (THE retry policy), `data/db.py` (one SQLAlchemy engine), `data/freshness.py` (TTL checks), `data/loaders.py` (`load_fdic_hist`), `data/cache.py` (Postgres in prod / sqlite locally), `utils/formatting` (numeric formatters), `data/events/wire_base.is_junk_news` (one junk filter), `utils/chart_style.py` + `ui/styles.py` `:root` tokens (charts/CSS), `ui/export.py` (THE table export: one .xlsx per table via `table_export(df, filename, key, formats=, row_formats=, provenance=)` — raw numbers + Excel formats, `n/a` for absent, Source sheet; FDIC $K columns must say `($K)`; `row_formats=` keyed by the line-item label for metric-in-rows tables; never write a per-page openpyxl copy or a CSV).
