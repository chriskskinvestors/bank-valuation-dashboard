...
with BOTH sub-tabs (Detailed M&A History + the kept Insider Activity), and
the M&A pane's bank picker in its no-selection state (index=None — no
network fan-out happens until a bank is picked, so this test never touches
FDIC/EDGAR).
...
class TestTransactionsSection(unittest.TestCase):
    ...
    def test_subtabs_and_bank_picker_render(self):
        ...
        tab_bar = next((r for r in at.radio
                        ...
                        ["Detailed M&A History", "Insider Activity"]), None)
        self.assertIsNotNone(
            tab_bar,
            "Transactions lazy_tabs bar missing. Radios: "
            f"{[list(r.options) for r in at.radio]}")
        picker = next((sb for sb in at.selectbox if sb.key == "txn_ma_bank"), None)
        self.assertIsNotNone(picker, "M&A History bank picker missing")
