...
def main() -> int:
    ...
    for adapter in adapters:
        ...
        try:
            ...
            # Timestamp repair (2026-09-14, extended 2026-09-16): existing
            # rows keep their stamp on conflict-ignore inserts, so converge
            # them to the adapter's freshly computed published_at. sec_8k:
            # the acceptance-time fix. fmp_news: the naive-stamp-is-ET fix
            # (rows were 4-5h stale, sinking fresh release-window PRs — the
            # BCBP offering case). No-ops once converged.
            if adapter.name in ("sec_8k", "fmp_news"):
                from data.events.store import restamp_events
                n_restamped = restamp_events(events)
                if n_restamped:
                    print(f"  [{adapter.name}] restamped {n_restamped} "
                          "stored rows to corrected publish time")
        except TimeoutError:
            timeouts += 1
